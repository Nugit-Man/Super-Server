...
execute if score 1 GB_Select matches 1 run execute as @a[scores={GB_Mode=1,MAIN_Game=3},tag=GB_shrink] run execute as @s[tag=GB_knockback] run execute as @s[tag=GB_elytra] run advancement grant @s[tag=GB_dash,scores={GB_Points=4}] only gorbino:agile
...
#yeah, life has its way of running things