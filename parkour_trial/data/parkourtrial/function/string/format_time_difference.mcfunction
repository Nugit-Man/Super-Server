# Formats the difference between @s's time and the record.
scoreboard players operation ticks pt_globals = @s pt_current_time
scoreboard players operation ticks pt_globals -= record_ticks pt_globals
...
data modify storage parkourtrial:scratchpad finish_difference_string set from storage parkourtrial:scratchpad formatted_time
execute if score negative pt_globals matches 1 run data modify storage parkourtrial:scratchpad finish_difference set value \
    [{color:"green",text:"(-"},{type:"nbt",storage:"parkourtrial:scratchpad",nbt:"finish_difference_string",interpret:true},")"]
execute if score negative pt_globals matches 0 run data modify storage parkourtrial:scratchpad finish_difference set value \
    [{color:"red",text:"(+"},{type:"nbt",storage:"parkourtrial:scratchpad",nbt:"finish_difference_string",interpret:true},")"]
