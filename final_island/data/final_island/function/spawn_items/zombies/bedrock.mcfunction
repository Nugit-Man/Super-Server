...
execute if score FI_Zombie_Bedrock FI_Item_Cap matches 0..17 in final_island:game if score FI_Zombie_Bedrock FI_Random matches 1 at @e[type=marker,limit=1,sort=random,tag=FI_Zombie_Bedrock_Spawner] run summon zombie ~ ~ ~ {Tags:[FI_Zombie_Bedrock,FI_Zombie_Armor_Bedrock],drop_chances:{feet:0,legs:0,chest:0,head:0,mainhand:0,offhand:0}}
